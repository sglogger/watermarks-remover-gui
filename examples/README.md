...
| `sample-marked.docx` | Invisible characters in the body text plus AI markers in the document properties (generated, see below) | metadata and body text cleaned; verified clean. Under v0.5.0 this one stayed flagged with nothing removed |
| `sample-clean.txt` | Nothing unusual | reported clean, which is worth seeing too |

The `.md` and `.svg` rows are the interesting ones, and they are kept
deliberately, because a tool that only ever demonstrates success teaches you
nothing about where it stops. They fail in opposite directions, which is the
point: the Markdown file is reported as still flagged when a character was left
behind on purpose, while the SVG is reported as clean when two characters were
